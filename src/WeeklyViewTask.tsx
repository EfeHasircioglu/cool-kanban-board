...
export default function WeeklyViewTask({ task }: any) {
  const { setEditingTask, setIsEditOpen, removeTask }: any = useTasks();
  return (
    <>
      <div className="flex relative flex-col gap-1 p-2 dark:bg-gray-950 dark:text-white bg-white w-full shadow-lg rounded-lg">
        <div className="absolute right-0 h-min w-min border-none outline-none">
          <Menu>
            <MenuButton>
              <svg
                className="absolute top-0 right-2 p-1 w-6 h-6 rounded-lg cursor-pointer dark:active:bg-white/15 dark:hover:bg-white/10 active:bg-black/15 hover:bg-black/10" /*  */
        ...
        <div className="font-bold pr-5">{task.title}</div>
        <div className="text-xs font-bold p-1 px-3 rounded-full dark:bg-white dark:text-black bg-black w-fit text-white">
        ...
        {task.description !== "" && (
          <Disclosure as="div" defaultOpen={false}>
            <DisclosureButton>
              <span className="text-xs flex flex-row gap-1 p-1 rounded-lg dark:hover:bg-white/10 dark:active:bg-white/15 hover:bg-black/10 active:bg-black/15 cursor-pointer px-1">
                Description
                <div>
                  <svg
                    className="w-4 h-4"
            ...
            <DisclosurePanel>
              <div className="text-sm p-1 font-bold">{task.description}</div>
            </DisclosurePanel>
          </Disclosure>
        )}
      </div>

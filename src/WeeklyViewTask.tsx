...
export default function WeeklyViewTask({ task }: any) {
  return (
    <div className="flex flex-col gap-1 p-2 bg-white w-full shadow-lg rounded-lg">
      <div className="font-bold">{task.title}</div>
      <div className="text-xs font-bold p-1 px-3 rounded-full bg-black w-fit text-white">
      ...
      {task.description !== "" && (
        <Disclosure as="div" defaultOpen={false}>
          <DisclosureButton>
            <span className="text-xs flex flex-row gap-1 p-1 rounded-lg hover:bg-black/10 active:bg-black/15 cursor-pointer px-1">
              Description
              <div>
                <svg
                  className="w-4 h-4"
